...
function universalDateTime(date) {
    ...
    var d = parseInt(date.substr(6, 13));
    var date = new Date(d);

    var day = date.getDate(); //Date of the month: 2 in our example
    //var monthnumber = addZero(date.getMonth() + 1); //Month of the Year: 0-based index, so add +1
    var name = monthNames[date.getMonth()]
    var year = date.getFullYear();
    var hr = addZero(date.getHours());
    var min = addZero(date.getMinutes());
    var dateTime = day + " " + name + " " + year + " @" + hr + ":" + min;
    return dateTime;
};
...
app.config(["$routeProvider", "$locationProvider",function ($routeProvider, $locationProvider) {
    ...
    .when('/O/:paperName', {
        templateUrl: 'OpinionList',
        controller: 'opinionListController'
    })
        .when('/R/:getOpinion', {
            templateUrl: 'OpinionRead',
            controller: 'opinionReadController'
        })
    .when('/Terms', {
        templateUrl: 'Terms',
    })
...
app.controller('opinionListController',["$scope", "$http","$routeParams","$q", function ($scope, $http, $routeParams,$q) {
    var paper = $routeParams.paperName;

    $scope.paper = paper.replace(/([a-z])([A-Z])/g, '$1 $2');
    $scope.$on('$viewContentLoaded', function () {
        $http({
            method: 'POST',
            url: "/Home/GetOpinionList",
            data: {
                PaperName: $scope.paper,
            },

        }).success(function (data) {
            $scope.articleList = data;
            for (i = 0; i < data.length; i++) {
                $scope.articleList[i].TimeStamp = dateOnly(data[i].TimeStamp);
                console.log($scope.articleList[i].TimeStamp);
            }
            console.log(data);
...
app.controller('opinionReadController',["$scope", "$http","$routeParams","$sce","$q", function ($scope, $http, $routeParams, $sce,$q) {
    var searchTitle = $routeParams.getOpinion;
    $scope.$on('$viewContentLoaded', function () {
        $http({
            method: 'POST',
            url: "/Home/GetOpinion",
            ...
            $scope.BaseImage = data.BaseImage;
            $scope.Content = $sce.trustAsHtml(data.Opinion);
            $scope.User = data.UModel;
            $scope.Tags = data.TModel;
            $scope.Data = data;
            $scope.linkModel = [];
            for (i = 0; i < data.RelatedTitle.length; i++) {
                $scope.linkModel[i] = {
                    "name": data.RelatedTitle[i],
                    "link": data.RelatedTitle[i].replace(/\s+/g, '-')
                };
            }
            $scope.GetComments();
        });
    ...
    $scope.GetComments = function () {
        $http({
            ...
            $scope.Comments = data;
            $scope.Comments.Count = data.length;
        });
    };
